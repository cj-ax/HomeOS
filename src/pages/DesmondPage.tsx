/**
 * DesmondPage — Age 9 Dashboard
 * Gamified layout: chores with points, leaderboard, countdowns,
 * reminders, weather, Spotify. Fire HD 8 (1280×800).
 */
...
const IC = {
  ...
  clock: (p: IconProps = {}) => <Sv {...p}><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></Sv>,
...
/* ── Mock Data ── */
const MOCK_CHORES = [
  { id: 1, name: 'Make bed', done: true, points: 5, emoji: '🛏️' },
  { id: 2, name: 'Feed the dog', done: true, points: 10, emoji: '🐕' },
  { id: 3, name: 'Clean up toys', done: false, points: 10, emoji: '🧸' },
  { id: 4, name: 'Set the table', done: false, points: 5, emoji: '🍽️' },
  { id: 5, name: 'Homework', done: false, points: 20, emoji: '📚' },
  { id: 6, name: 'Read 15 minutes', done: false, points: 10, emoji: '📖' },
];
...
const MOCK_COUNTDOWNS = [
  { label: 'Basketball game', time: 'Saturday 2:00 PM', hoursLeft: 52, color: C.orange },
  { label: 'Science fair', time: 'Next Friday', hoursLeft: 192, color: C.blue },
  { label: 'Spring break', time: 'Mar 28', hoursLeft: 240, color: C.green },
];

const MOCK_REMINDERS = [
  { time: '3:45 PM', label: 'Basketball practice', color: C.orange },
  { time: '6:00 PM', label: 'Shower time', color: C.blue },
];
...
const GREETINGS = [
  "Snowy day today — wear your warm jacket and snow boots!",
  "Basketball practice tonight — make sure to pack your gear!",
  "You're 4 days into your streak. Keep it going!",
  "Science fair is coming up — work on your project this week!",
];
...
const BroadcastOverlay = ({ message, onAcknowledge }: { message: string; onAcknowledge: () => void }) => (
  <div style={{
    ...
    <div style={{
      width: 72, height: 72, borderRadius: 22, background: `${C.accent}20`,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      {IC.megaphone({ sz: 36, c: C.accent })}
    </div>
    <div style={{ fontSize: 15, fontWeight: 700, color: C.t2, textTransform: 'uppercase', letterSpacing: 2 }}>
      Message from Mom & Dad
    </div>
    <div style={{
      fontSize: 26, fontWeight: 700, color: C.white, textAlign: 'center',
      maxWidth: 480, lineHeight: 1.4,
    }}>
      {message}
    </div>
    <button onClick={onAcknowledge} style={{
      marginTop: 16, padding: '16px 56px', borderRadius: 16,
...
export function DesmondPage() {
  ...
  useEffect(() => {
    if (!spPlaying) return;
    const i = setInterval(() => setSpPos((p) => (p >= 197 ? 0 : p + 1)), 1000);
    return () => clearInterval(i);
  }, [spPlaying]);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 22, fontWeight: 800 }}>{greeting}, Desmond</span>
            <Pill color={C.amber}>
              <IC.bolt sz={10} c={C.amber} />
              {earnedPoints} pts today
            </Pill>
          </div>
        ...
        {/* ── Greeting + Weather + Reminders + Countdowns ── */}
        <div style={{ gridArea: 'greeting', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Morning tip */}
          <Glass style={{ padding: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.amber, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>
              Today's Tip
            </div>
          ...
          <Glass style={{ padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              {IC.bell({ sz: 14, c: C.blue })}
              <span style={{ fontSize: 11, fontWeight: 700, color: C.t2 }}>REMINDERS</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {MOCK_REMINDERS.map((ev, i) => (
          ...
          <Glass style={{ padding: 14, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              {IC.clock({ sz: 14, c: C.green })}
              <span style={{ fontSize: 11, fontWeight: 700, color: C.t2 }}>COMING UP</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {MOCK_COUNTDOWNS.map((cd, i) => (
                <div key={i} style={{
                  display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px',
                  borderRadius: 10, background: `${cd.color}08`, border: `1px solid ${cd.color}20`,
                }}>
                  <span style={{
                    fontSize: 18, fontWeight: 800, color: cd.color,
                    minWidth: 40, textAlign: 'center',
                  }}>
                    {cd.hoursLeft < 24 ? `${cd.hoursLeft}h` : `${Math.floor(cd.hoursLeft / 24)}d`}
                  </span>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: C.t1 }}>{cd.label}</div>
                    <div style={{ fontSize: 10, color: C.t3 }}>{cd.time}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Glass style={{ gridArea: 'chores', padding: 14, overflow: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {IC.star({ sz: 14, c: C.amber })}
              <span style={{ fontSize: 11, fontWeight: 700, color: C.t2 }}>QUEST LOG</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 11, color: C.t3 }}>{doneCount}/{chores.length}</span>
              <Pill color={C.green}>+{earnedPoints}/{totalPoints}</Pill>
            </div>
          </div>

          {/* XP bar */}
          <div style={{ marginBottom: 4 }}>
            <Bar value={doneCount / chores.length} color={C.amber} h={8} />
          </div>
          <div style={{ fontSize: 9, color: C.t3, textAlign: 'right', marginBottom: 10 }}>
            {Math.round((doneCount / chores.length) * 100)}% complete
          </div>
            ...
            {chores.map((c) => (
              ...
              >
                ...
                <div style={{
                  ...
                  <IC.bolt sz={10} c={c.done ? C.green : C.amber} />
                  <span style={{ fontSize: 11, fontWeight: 700, color: c.done ? C.green : C.amber }}>{c.points}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {doneCount === chores.length && (
            <div style={{
              ...
              border: `1px solid ${C.amber}30`, textAlign: 'center',
            }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>🏆</div>
              <span style={{ fontSize: 14, fontWeight: 700, color: C.amber }}>
                All quests complete! +{totalPoints} XP earned!
              </span>
            </div>
          )}
        </Glass>
        ...
        <Glass style={{ gridArea: 'music', padding: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 14, flex: 1 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 6 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.white }}>Thunder</div>
              <div style={{ fontSize: 11, color: C.t2 }}>Imagine Dragons</div>

              <div style={{ marginTop: 4 }}>
                <Bar value={spPos / 197} color={C.green} h={3} />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                  <span style={{ fontSize: 9, color: C.t3, fontVariantNumeric: 'tabular-nums' }}>{fmtTime(spPos)}</span>
                  <span style={{ fontSize: 9, color: C.t3, fontVariantNumeric: 'tabular-nums' }}>{fmtTime(197)}</span>
                </div>
              </div>
        ...
        <div style={{ gridArea: 'sidebar', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {sorted.map((kid, i) => (
                <div key={kid.name} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 13, fontWeight: 700, color: kid.color,
                  }}>
                    {kid.name[0]}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: kid.name === 'Desmond' ? C.white : C.t1 }}>
                      {kid.name}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
                      <IC.flame sz={10} c={C.amber} />
                      <span style={{ fontSize: 9, color: C.t3 }}>{kid.streak} days</span>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ fontSize: 15, fontWeight: 800, color: kid.color }}>{kid.points}</span>
                    <div style={{ fontSize: 8, color: C.t3, fontWeight: 600 }}>XP</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop: 14, padding: '10px 12px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: `1px solid ${C.border}` }}>
              ...
              {sorted.map((kid) => (
                <div key={kid.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 10, color: C.t2, width: 55, flexShrink: 0 }}>{kid.name}</span>
          ...
          >
            <div style={{
              ...
              {IC.send({ sz: 20, c: C.accent })}
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700 }}>Message Mom & Dad</div>
